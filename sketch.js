...
const densePoseURL = 'http://localhost:8000/query';
let video;
let densePoseImage;
let myCanvas;
let startBTN;
...
function draw() {
  ...
  translate(640, 0);
  scale(-1.0, 1.0);
  if (densePoseImage) image(densePoseImage, 0, 0, 640, 480);
...
function getDensePoseImage() {
  if (video && video.elt) {
    ...
    httpPost(densePoseURL, 'json', postData, async (output) => {
      if (output && output.image) {
        // createImg(output.image);
        densePoseImage = await loadImage(output.image);
        if (isStarting) getDensePoseImage();
      }
    })
  }
}
